...
export default function MateSetup() {
  ...
  return <main style={{maxWidth:760,margin:'40px auto',padding:24,color:'#f9edff',lineHeight:1.7}}>
    ...
    <p>{config.enabled && config.provisioningReady ? 'Login is configured. Test password login and password setup from the homepage.' : 'Real login is not configured yet. No simulated login or account bypass is available.'}</p>
    ...
    <ol>
      ...
      <li>In Supabase, allow your exact app origin plus <code>/auth/reset-password</code> as a redirect. Use the standard recovery email template with its ConfirmationURL.</li>
      ...
      <li>On the homepage, select LOG IN and RESET PASSWORD for first-time setup. Open the email in this browser, save your password, then use email + password for future logins.</li>
      <li>Check the camera reveal, the live site content on the glass, and LOG OUT returning to the public homepage.</li>
    </ol>
    <p>SIGN UP saves MailerLite membership, then sends a RESET PASSWORD email when entry is enabled. The reset form stays open until you save your password.</p>
