...
export const ExteriorHull = memo(function ExteriorHull({config,hull,view}:{config:DomeConfig;hull:HullConfig;view:Viewport}) {
  const patches = hullMesh(config,hull).map(p=>({...p,depth:p.points.reduce((sum,point)=>sum+project(point,config,view).depth,0)/4})).sort((a,b)=>b.depth-a.depth);
