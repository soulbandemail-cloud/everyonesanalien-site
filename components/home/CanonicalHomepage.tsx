...
import { MateLogin } from '@/components/mate/MateLogin';
...
export default function CanonicalHomepage({ cockpit, loginEnabled, config, view, animateEntry = false, minigameEnabled = !cockpit }: { cockpit: boolean; loginEnabled: boolean; config: DomeConfig; view: Viewport; animateEntry?: boolean; minigameEnabled?: boolean }) {
...
return (
  <div ref={homeRoot} className="canonical-home">
    {minigameEnabled && <>
    <div
...
<div className="shooting-stars">
  ...
  <span className="shooting-star shooting-star-10" onPointerDown={catchShootingStar}></span>
      ...
      {cockpit && <dialog ref={videoDialog} className="mate-video-dialog" aria-label="SOUL music video" onClose={()=>setVideoOpen(false)}>
        <button type="button" onClick={()=>videoDialog.current?.close()}>CLOSE VIDEO</button>
        {videoOpen && television}
      </dialog>}
  ...
  onClick={minigameEnabled ? toggleUfoOrbit : undefined}
>
                  <defs>
      ...
      >
        ...
        <div className="grid gap-8 md:gap-16 md:grid-cols-3 mt-2 mb-4 md:mb-16">
          <section data-dome-slot="mate" className="md:max-w-sm md:mx-auto">
            ...
            <MateLogin enabled={loginEnabled} authenticated={cockpit} />
